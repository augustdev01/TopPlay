...
export default function AdminTransactionsPage() {
  ...
  const getSourceLabel = (source: string) => {
    switch (source) {
      case 'callback': return 'Callback Wave';
      case 'user_input': return 'Saisie utilisateur';
      case 'admin': return 'Validation admin';
      case 'api_verification': return 'API Wave';
  ...
  const handleConfirmTransaction = (transactionId: string) => {
    setTransactions(prev => 
      prev.map(t => 
        t._id === transactionId 
          ? { ...t, status: 'confirmed', validatedAt: new Date().toISOString() }
          : t
      )
    );
  };
  ...
  const handleRejectTransaction = (transactionId: string) => {
    setTransactions(prev => 
      prev.map(t => 
        t._id === transactionId 
          ? { ...t, status: 'rejected', validatedAt: new Date().toISOString() }
          : t
      )
    );
  };
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Gérez et validez les paiements</p>
          </div>
        ...
        <Card className="rounded-2xl shadow-lg border-0">
          <CardContent className="p-6 text-center">
            ...
            <div className="text-2xl font-bold text-indigo-600">
              {(transactions.reduce((acc, t) => acc + (t.status === 'confirmed' ? t.amount : 0), 0) / 1000).toFixed(0)}K
            </div>
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <div className="flex items-center space-x-3 mb-2">
                          <h3 className="font-bold text-gray-900">{transaction.playerName}</h3>
                          <Badge className={`${statusConfig.color} rounded-full px-3 py-1 text-xs`}>
                            {statusConfig.label}
                          </Badge>
                        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-600">
                          <div>
                            <span className="font-medium">Compétition:</span> {transaction.competition}
                          </div>
                          <div>
                            <span className="font-medium">Montant:</span> {transaction.amount} FCFA
                          </div>
                          <div>
                          ...
                          <div>
                            <span className="font-medium">Téléphone:</span> {transaction.customerPhone}
                          </div>
                          <div>
                            <span className="font-medium">Source:</span> {getSourceLabel(transaction.source)}
                          </div>
                          <div>
                            <span className="font-medium">Créé:</span> {new Date(transaction.createdAt).toLocaleString('fr-FR')}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col space-y-2">
                      ...
                      {transaction.status === 'pending' && (
                        <>
                          <Button 
                            size="sm" 
                            onClick={() => handleConfirmTransaction(transaction._id)}
  ...
  function handleRejectTransaction(transactionId: string) {
    setTransactions(prev => 
      prev.map(t => 
        t._id === transactionId 
          ? { ...t, status: 'rejected', validatedAt: new Date().toISOString() }
